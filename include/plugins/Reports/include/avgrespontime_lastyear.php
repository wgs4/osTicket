            ...
            ".TABLE_PREFIX."ticket.ticket_id = ".TABLE_PREFIX."thread.object_id
             LEFT JOIN ".TABLE_PREFIX."thread_entry on
                  ".TABLE_PREFIX."thread.id = ".TABLE_PREFIX."thread_entry.thread_id
                       AND ".TABLE_PREFIX."thread_entry.type IN ('R') /* N is notes, R is replys */ AND YEAR(".TABLE_PREFIX."ticket.created)=".date('Y',strtotime("-1 year"))." 
...
echo "<div class='quick quick_left'>".date('Y',strtotime("-1 year"))."</div><div class='quick quick_right'>".Report::duration($timeInSeconds)."</div>";
